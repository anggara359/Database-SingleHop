...
const uint64_t beforeModuleId2 = 0xF0F0F0F0E3LL;
const uint64_t beforeModuleId3 = 0xF0F0F0F0E4LL;
const uint64_t moduleId = 0xF0F0F0F0E5LL;
...
int dataCounterModul1 = 0;
int dataCounterDisplayModul1 = 0;
int dataCounterModul2 = 0;
int dataCounterDisplayModul2 = 0;
int dataCounterModul3 = 0;
int dataCounterDisplayModul3 = 0;
int dataCounterModul4 = 0;
//int dataCounterNewCustomer = 0;
//int dataCounterNewCustomermodul1 = 0;
...
void setup() {
  ...
  void lcd() {
    ...
    display.println(dataCounterDisplayModul3);
...
void bacanrf() {
  radio.openReadingPipe(1, beforeModuleId);
  radio.openReadingPipe(2, beforeModuleId2);
  radio.openReadingPipe(2, beforeModuleId3);
  ...
  if (radio.available()) {
    ...
    if (customerId == 19466411ULL) {
      dataCounterDisplayModul1++;
    }
    if (customerId == 19466422ULL) {
      dataCounterDisplayModul2++;
    }
    if (customerId == 19466433ULL) {
      dataCounterDisplayModul3++;
    }
...
void kirimnrf2() {
  ...
  const uint64_t newCustomerId = 19466444ULL;
...
void loop() {
  ...
  Serial.print("Jumlah data diterima (ID 19466433): ");
  Serial.println(dataCounterDisplayModul3);

  if (ct - rtimer0 >= timer0 >= interval && dataCounterModul3 < 8) {
    kirimnrf();
    rtimer0 = ct;
  }

  if (ct2 - rtimer1 >= timer1 >= interval && dataCounterModul4 < 8) {
    kirimnrf2();
    rtimer1 = ct2;
  }
