...
const uint64_t otherModuleId4 = 0xF0F0F0F0F3LL;
...
bool isReqConnModeSending = false;
bool isReqConnModeReceive = true;
...
//    delay(100);
  ...
  if (radio.available() && isReqConnModeReceive) {
    Serial.println("Receiving...");
    if(moduleType == 3) radio.openReadingPipe(1, otherModuleId); //to
    if(moduleType == 4) radio.openReadingPipe(2, otherModuleId4); //to
//    delay(10);
...
//    payloadReqConn.counter = received.counter;       // save incoming counter for next outgoing counter
    ...
    if(strcmp(received.param,"PING") == 0 && moduleType == 3) {
...
//      delay(100);
//      digitalWrite(module2Pin, LOW);
    }else if(strcmp(received.param,"PONG") == 0 && moduleType == 4) {
      isReqConnModeSending = false;
      isReqConnModeReceive = false;
...
void setup() {
  ...
  radio.openReadingPipe(2, otherModuleId4); //to
...
//  CustomerId,UserTotalUsage,UserTotalCost,CurrentDate,CurrentTime
  byte newData[30];
...
//    Serial.println(dataCounterNewCustomer);
    ...
    if (currentMillis - previousCT1Millis >= intervalCT1 && dataCounter3 < 30) {
      kirimnrf();
      previousCT1Millis = ct;
      Serial.println("[1] senddata to another module");
    }
  
    if (currentMillis2 - previousCT2Millis >= intervalCT2 && dataCounter2 < 30) {
      kirimnrf2();
