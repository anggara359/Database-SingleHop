...
RF24 radio(10, 9);
const uint64_t moduleId1 = 0xF0F0F0F0E2LL; // module 1
const uint64_t moduleId2 = 0xF0F0F0F0E3LL; // module 2 
const uint64_t moduleId3 = 0xF0F0F0F0E4LL; // module 3
const uint64_t moduleId4 = 0xF0F0F0F0F3LL; // module 4

int moduleType = 1; //from 1
int moduleSelfNumber = 2; //module number
int moduleCommNumber = 0; //module can communicate to ...
...
void sendMsg(String msg) {
  ...
  memcpy(payloadReqConn.param, msg, msgLength + 1);  // set the outgoing message
  radio.stopListening();
...
//    digitalWrite(module2Pin, LOW);
      ...
      if(moduleCommNumber == 1) radio.openWritingPipe(moduleId1);
      if(moduleCommNumber == 2) 
//    delay(10);
//    byte data[50];
...
//      isReqConnModeReceive = false;
    ...
    if(moduleType == 2) { //module2
      sendMsg("PONG");
      
//      isReqConnModeSending = false;//f
//      isReqConnModeReceive = true;//ttrue
    }
    if(moduleType == 3) { //module3
      sendMsg("PONG");
      
//      isReqConnModeSending = false;//f
//      isReqConnModeReceive = true;//true
    }
    if(moduleType == 4) { //module3
      sendMsg("PONG");
...
//      isReqConnModeReceive = true;//true
  ...
  if (radio.available() && isReqConnModeReceive) {
    ...
    if(moduleType == 1) radio.openReadingPipe(1, moduleId1);
    if(moduleType == 2) radio.openReadingPipe(2, moduleId2);
    if(moduleType == 3) radio.openReadingPipe(3, moduleId3);
    if(moduleType == 4) radio.openReadingPipe(4, moduleId4);
//    delay(10);
